  ...
  LinuxBinary(instance,
    function(error, binary) {
      ...
      if (instance.options.env) {
        command.push(`env ${instance.options.env}`);
      }
...
function MacCommand(instance, end) {
  ...
  // Added ENV
  const command = instance.options.env 
    ? `${instance.options.env} ${instance.command}`
    : instance.command
...
module.exports.exec = Node.util.promisify(Exec);
